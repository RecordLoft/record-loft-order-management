...
export const PRODUCTS_LIST_QUERY = `#graphql
  query ProductsListPage($cursor: String, $query: String!) {
...
const FIND_SHOP_CHANNEL = `#graphql
  query FindShopChannel {
    channels(first: 25) {
      nodes {
        name
        handle
...
const DEFAULT_SHOP_CHANNEL_NAME = "Shop";
/** Online Store channel app id (used when channels query returns no match). */
const DEFAULT_ONLINE_STORE_APP_ID = "580111";

function numericIdFromGid(gid: string): string {
  const match = /(\d+)$/.exec(gid);
  if (!match) {
    throw new Error(`Could not parse id from ${gid}`);
  }
  return match[1];
}

function resolveShopChannelAppId(
  channels: { name: string; handle: string; app: { id: string } | null }[],
  channelName: string,
): string | null {
  const nameLower = channelName.toLowerCase();
  const channel =
    channels.find((c) => c.name.toLowerCase() === nameLower) ??
    channels.find((c) => c.handle.toLowerCase() === "shop") ??
    channels.find((c) => c.handle.toLowerCase() === "online-store") ??
    channels.find((c) => c.name.toLowerCase().includes("online store"));
  const appGid = channel?.app?.id;
  return appGid ? numericIdFromGid(appGid) : null;
}

/**
 * Products query filter for items published on the Shop sales channel.
 * Override with PUBLISHED_STATUS_QUERY (e.g. published_status:580111-published).
 */
export async function resolveShopPublishedStatusFilter(
  graphql: GraphqlRequest,
): Promise<string> {
  const override = process.env.PUBLISHED_STATUS_QUERY?.trim();
  if (override) return override;

  const envAppId = process.env.SHOP_CHANNEL_APP_ID?.trim();
  if (envAppId) {
    return `published_status:${envAppId}-published`;
  }

  const channelName =
    process.env.SHOP_PUBLICATION_NAME?.trim() ||
    process.env.SHOP_CHANNEL_NAME?.trim() ||
    DEFAULT_SHOP_CHANNEL_NAME;

  const response = await graphql(FIND_SHOP_CHANNEL);
  const json = (await response.json()) as {
    data?: {
      channels?: {
        nodes: {
  ...
  if (json.errors) {
    throw new Error(
      `Failed to list channels: ${JSON.stringify(json.errors)}`,
    );
  }

  const channels = json.data?.channels?.nodes ?? [];
  const appId =
    resolveShopChannelAppId(channels, channelName) ?? DEFAULT_ONLINE_STORE_APP_ID;

  return `published_status:${appId}-published`;
}
...
/** Products published on the Shop sales channel (see resolveShopPublishedStatusFilter). */
export async function listShopPublishedProductGids(
  graphql: GraphqlRequest,
): Promise<{ id: string; title: string }[]> {
  const query = await resolveShopPublishedStatusFilter(graphql);
  return listAllProductGids(graphql, { query });
}
